...
timeout = int(settings_Controller.device_environ.get("CONNECTION_TIMEOUT", None))
assert timeout, "Connection timeout must be entered in CONNECTION_TIMEOUT"
wifi_ssid = settings_Controller.device_environ.get("WIFI_SSID", None)
assert wifi_ssid, "Wi-Fi SSID must be entered in WIFI_SSID"
wifi_passw = settings_Controller.device_environ.get("WIFI_PASSW", None)
...
settings_Controller.device_environ["DEVICE_IP"] = device_ip
server_ip = None
...
async def start_all():
    device_port = settings_Controller.device_environ.get("DEVICE_PORT", None)
    ...
    sock.settimeout(350)
    while True:
        server_ip = settings_Controller.server_environ.get("SERVER_IP", None)
        server_port = settings_Controller.server_environ.get("SERVER_PORT", None)
